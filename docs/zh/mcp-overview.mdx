...
只有当确切的 `METHOD /api/...` 路径声明在某个工具的注册表 `_apiPaths` 条目中时，该 API 端点才算**由 MCP 暴露**。下表是这些声明来自 `api/mcp/registry/_cache-tools.ts` 和 `api/mcp/registry/_rpc-tools.ts` 的面向人类的呈现；它比公共 OpenAPI 目录更窄。
...
4. 如需代码级验证，请在 `api/mcp/registry/_cache-tools.ts` 和 `api/mcp/registry/_rpc-tools.ts` 中搜索 `_apiPaths`；parity 测试会解释有意的仅限 REST 排除项。
