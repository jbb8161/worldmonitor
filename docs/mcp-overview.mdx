...
An API endpoint is **MCP-exposed** only when the exact `METHOD /api/...` path is declared in a tool's registry `_apiPaths` entry. The table below is the human-facing rendering of those declarations from `api/mcp/registry/_cache-tools.ts` and `api/mcp/registry/_rpc-tools.ts`; it is narrower than the public OpenAPI catalog.
...
4. For code-level verification, search `_apiPaths` in `api/mcp/registry/_cache-tools.ts` and `api/mcp/registry/_rpc-tools.ts`; the parity test explains intentional REST-only exclusions.
